...
Mean relative viability is seen in @fig-viability-relative-overall.

{{< embed notebooks/viability/Viability.qmd#fig-viability-relative-overall >}}

The individual viability for each donor is seen in @fig-donor-viabilities

{{< embed notebooks/viability/Viability.qmd#fig-donor-viabilities >}}

Absolute overall viability is seen in @fig-viability-absolute-overall

{{< embed notebooks/viability/Viability.qmd#fig-viability-absolute-overall >}}

Morphology of the spheroids over time is seen in @fig-morph

![](images/morphology_AS0006.png){#fig-morph}

### mRNA

{{< embed notebooks/qPCR/mRNA.qmd#fig-mrna-3a4-il6 >}}

{{< embed notebooks/qPCR/mRNA.qmd#fig-mrna-120hours >}}

### Activity

```{r}
#| label: mean of the two donors

data_act <- readRDS("notebooks/LCMS/data_processed/final_data.rds")

mean_act_120h_IL6_0.001 <- 
  data_act[treatment == "0.01 ng/ml IL-6" 
           & time_incubation == 0.5
           & time_treatment == 120, mean(mean_relative_amount)]

mean_act_120h_IL6_0.001 <- round(mean_act_120h_IL6_0.001, digits = 2) *100
```

We see a marked decrease in 3A4 activity with high concentrations of IL-6, which corresponds well with the litterature. At lower concentrations we see a much more modest decrease in activity with an activity of `r mean_act_120h_IL6_0.001`% relative to vehicle control

Consider looking into outliers in 120 hour 0.01 ng-IL1B, and 120 hour 0.1 ng-IL6.
